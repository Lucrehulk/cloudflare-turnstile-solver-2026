...
| Solver | `255` | Just a dummy ping packet. I've encountered issues with socket connections being closed, which is seemingly from inactivity. So solvers occasionally send this (every 15s as of now) just to keep the connection alive so the browser doesn't close it. **Structure:** `<255>` |
